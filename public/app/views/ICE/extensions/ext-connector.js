...
methodDraw.addExtension("Connector", function (S) {
  ...
  function setPoint(elem, pos, x, y, setMid) {
    ...
    if (setMid) {
      ...
      setPoint(
        elem,
        1,
        (pt_end.x + pt_start.x) / 2,
        (pt_end.y + pt_start.y) / 2,
      );
    }
  }
  ...
  function updateLine(diff_x, diff_y) {
    ...
    while (i--) {
      ...
      var pt2 = getBBintersect(
        pt.x,
        pt.y,
        elData(line, alt_pre + "_bb"),
        getOffset(alt_pre, line),
      );
  ...
  return {
    ...
    mouseDown: function (opts) {
      var e = opts.event;
      (start_x = opts.start_x), (start_y = opts.start_y);
        ...
        if ($.inArray(svgcontent, parents) != -1) {
          ...
          cur_line = addElem({
            ...
            attr: {
              id: getNextId(),
              points:
                x + "," + y + " " + x + "," + y + " " + start_x + "," + start_y,
              stroke: "#" + curConfig.initStroke.color,
              "stroke-width":
                !start_elem.stroke_width || start_elem.stroke_width == 0
                  ? curConfig.initStroke.width
                  : start_elem.stroke_width,
              fill: "none",
    ...
    mouseMove: function (opts) {
      ...
      if (mode == "connector" && started) {
        ...
        var pt = getBBintersect(
          x,
          y,
          elData(cur_line, "start_bb"),
          getOffset("start", cur_line),
        );
        start_x = pt.x;
        start_y = pt.y;
