...
(function () {
  ...
  function linkFileSelect(
    ...
    function bindAttrToFileInput(fileElem) {
      ...
      for (var i = 0; i < elem[0].attributes.length; i++) {
        ...
        if (
          (isInputTypeFile() && attribute.name !== "type") ||
          (attribute.name !== "type" &&
    ...
    function clickHandler(evt) {
      ...
      if (fileElem) {
        ...
        if (
          evt &&
          resetOnClick &&
          $parse(attr.ngfResetModelOnClick)(scope) !== false
        )
          resetModel(evt);
  ...
  ngFileUpload.validate = function (scope, $parse, attr, file, evt) {
    ...
    var fileSizeMax =
      $parse(attr.ngfMaxSize)(scope, { $file: file, $event: evt }) ||
      9007199254740991;
    var fileSizeMin =
      $parse(attr.ngfMinSize)(scope, { $file: file, $event: evt }) || -1;
    if (accept != null && angular.isString(accept)) {
      var regexp = new RegExp(globStringToRegex(accept), "gi");
      accept =
        (file.type != null && regexp.test(file.type.toLowerCase())) ||
        (file.name != null && regexp.test(file.name.toLowerCase()));
    }
    return (
      (accept == null || accept) &&
      (file.size == null ||
        (file.size < fileSizeMax && file.size > fileSizeMin))
    );
  };

  ngFileUpload.updateModel = function (
    $parse,
    $timeout,
    scope,
    ngModel,
        ...
        extractFiles(
          ...
          function (files, rejFiles) {
            updateModel(
              ...
              ngModel,
              attr,
              attr.ngfChange || attr.ngfDrop,
              files,
              rejFiles,
              evt,
            );
          },
          $parse(attr.ngfAllowDir)(scope) !== false,
          attr.multiple || $parse(attr.ngfMultiple)(scope),
        );
      },
      false,
    );
