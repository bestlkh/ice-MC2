angular
  ...
  .controller(
    "classController",
    function ($scope, $rootScope, $routeParams, $window, $socket, $location) {
      ...
      $scope.checkInclude = function (student) {
        ...
        for (var header in student) {
          ...
          if (!student[header].indexOf)
            return student[header] === ($scope.search.value === "true");
          if (student[header].indexOf($scope.search.value) !== -1) return true;
        }
      };
      ...
      $scope.startStatus = function () {
        var socket = io(
          $location.host() +
            ...
            "/" +
            $rootScope.user.username,
          { timeout: 5000, reconnectionAttempts: 10, reconnectionDelay: 2000 },
        );
