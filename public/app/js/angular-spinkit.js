...
angular
  .module("ngRotatingPlaneSpinner", [])
  .directive("rotatingPlaneSpinner", function () {
    return {
      restrict: "E",
...
angular
  .module("ngDoubleBounceSpinner", [])
  .directive("doubleBounceSpinner", function () {
    return {
      restrict: "E",
...
angular
  ...
  .directive("wanderingCubesSpinner", function () {
    return {
      restrict: "E",
...
angular
  .module("ngChasingDotsSpinner", [])
  .directive("chasingDotsSpinner", function () {
    return {
      restrict: "E",
...
angular
  .module("ngThreeBounceSpinner", [])
  .directive("threeBounceSpinner", function () {
    return {
      restrict: "E",
...
angular
  .module("ngWordPressSpinner", [])
  .directive("wordPressSpinner", function () {
    return {
      restrict: "E",
...
angular
  .module("ngFadingCircleSpinner", [])
  .directive("fadingCircleSpinner", function () {
    return {
      restrict: "E",
...
angular
  .module("ngSpinkitImagePreloader", [])
  .directive("spinkitImagePreloader", [
    "$compile",
    "$injector",
    "$rootScope",
    function ($compile, $injector, $rootScope) {
      return {
        restrict: "A",
        scope: {
          ngSrc: "@",
        ...
        link: function (scope, element, attrs) {
          var spinnerWrapper,
            spinnerWrapperClass =
              scope.spinkitImagePreloaderClass || "spinner-wrapper",
            spinner;

          // Check for the existence of the spinkit-directive
          if (
            !$injector.has(
              attrs.$normalize(scope.spinkitImagePreloader) + "Directive",
            )
          )
            return;

          // Create and configure DOM-spinner-elements
          (spinnerWrapper = angular
            .element("<div/>")
            .addClass(spinnerWrapperClass)),
            (spinner = $compile("<" + scope.spinkitImagePreloader + "/>")(
              scope,
            ));
          ...
          if (attrs.width) spinnerWrapper.css("width", attrs.width + "px");

          if (element.css("height"))
            spinnerWrapper.css("height", element.css("height"));

          if (attrs.height) spinnerWrapper.css("height", attrs.height + "px");
...
angular.module("angular-spinkit").run([
  "$templateCache",
