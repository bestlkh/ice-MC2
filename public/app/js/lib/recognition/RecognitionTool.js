  ...
  _createClass(RecognitionTool, null, [
    {
      ...
      value: function hor(ls, s) {
        if (
          ls[s].type === SymbolTypes.FRACTION ||
          (ls[s].type === SymbolTypes.BRACKET &&
            ls[s].bracketType == BracketTypes.OPEN) ||
        ...
        while (index < ls.length) {
          if (
            RecognitionTool.isInRegion(ls[s].wall, ls[index]) &&
            !ls[index].marked
          ) {
            if (ls[s].maxX <= ls[index].minX) {
              if (
                ls[s].minY + (2 / 5) * ls[s].height <= ls[index].y &&
        ...
        while (queue.length != 0) {
          while (queue.length != 0) {
            ...
            while (temp2 != -1) {
              ...
              if (
                ls[temp2].type === SymbolTypes.LIMIT &&
                (ls[temp1].type === SymbolTypes.BRACKET ||
                  ls[temp1].type === SymbolTypes.FRACTION)
              ) {
            ...
            while (stack.length != 0) {
              ...
              var right =
                si == parent.symbols.length - 1
                  ? symbol.wall.right
                  : parent.symbols[si + 1].minX;
              var left = symbol.minX;
              if (symbol.type === Constant.SYMBOL_TYPES.LIMIT) {
                if (si == 0) {
                  left = wall.left;
                } else if (
                  parent.symbols[si - 1].type === Constant.SYMBOL_TYPES.BRACKET
                ) {
                  left = parent.symbols[si - 1].maxX;
                } else if (
                  parent.symbols[si - 1].type === Constant.SYMBOL_TYPES.FRACTION
                ) {
                  left = parent.symbols[si - 1].maxX;
                }
              }
              ...
              var regions = [
                ...
                RegionTypes.TLEFT,
                RegionTypes.BLEFT,
                RegionTypes.CONTAINS,
              ];
              for (var i = 0; i < regions.length; i++) {
                symbol.region[regions[i]].setWall(region_walls[i]);
                temp2 = RecognitionTool.start(
                  ls,
                  symbol.region[regions[i]].wall,
                );
                if (temp2 != -1) {
      ...
      value: function getTex() {
        ...
        if (type === SymbolTypes.LIMIT) {
          ...
          if (bst.hasAnyBottom())
            result +=
              "_{" +
              RecognitionTool.getTex(bst.region[RegionTypes.BLEFT]) +
          ...
          result +=
            Constant.TEX_TEXT[value] +
            "{" +
            RecognitionTool.getTex(bst.region[RegionTypes.CONTAINS]) +
            "} ";
