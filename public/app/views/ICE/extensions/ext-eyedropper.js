  ...
  return {
    ...
    mouseDown: function (opts) {
      ...
      if (mode == "eyedropper" && target) {
        ...
        currentStyle.strokeOpacity =
          target.getAttribute("stroke-opacity") || 1.0;
        ...
        if (!opts.selectedElements.length) {
          //nothing selected, just update colors
          var fill = getPaint(
            currentStyle.fillPaint,
            currentStyle.fillOpacity * 100,
            "fill",
          );
          var stroke = getPaint(
            currentStyle.strokePaint,
            currentStyle.strokeOpacity * 100,
            "stroke",
          );
        ...
        if ($.inArray(opts.selectedElements.nodeName, ["g", "use"]) == -1) {
          ...
          opts.selectedElements.forEach(function (element) {
            if (currentStyle.fillPaint)
              change(element, "fill", currentStyle.fillPaint);
            if (currentStyle.fillOpacity)
              change(element, "fill-opacity", currentStyle.fillOpacity);
            if (currentStyle.strokePaint)
              change(element, "stroke", currentStyle.strokePaint);
            if (currentStyle.strokeOpacity)
              change(element, "stroke-opacity", currentStyle.strokeOpacity);
            if (currentStyle.strokeWidth)
              change(element, "stroke-width", currentStyle.strokeWidth);
            if (currentStyle.strokeDashArray)
              change(element, "stroke-dasharray", currentStyle.strokeDashArray);
            if (currentStyle.opacity)
              change(element, "opacity", currentStyle.opacity);
            if (currentStyle.strokeLinecap)
              change(element, "stroke-linecap", currentStyle.strokeLinecap);
