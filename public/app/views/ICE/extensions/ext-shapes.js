...
methodDraw.addExtension("shapes", function () {
  ...
  function loadLibrary(cat_id) {
    ...
    cur_lib = lib;
    if (!lib.buttons.length && (cat_id === "basicMath" || cat_id == "object"))
      makeButtonsMath(cat_id, lib);
  ...
  function makeButtons(cat, shapes) {
    ...
    var shape_icon = new DOMParser().parseFromString(
      '<svg xmlns="http://www.w3.org/2000/svg"><svg viewBox="' +
        vb +
        '"><text id="mb" font-family="monospace" font-size="300" y="300" x="20" fill-opacity="null" stroke-opacity="null" stroke-width="0" stroke="#000" fill="#000000">O</text></svg></svg>',
      "text/xml",
    );
    ...
    for (var id in data) {
      ...
      if (cat !== "basic" && cat !== "numerical")
        icon.find("text").attr("font-size", 350);
  ...
  return {
    ...
    callback: function () {
      ...
      $(document).on("mouseup", function (e) {
        if (
          e.target.id.indexOf("_show") !== -1 &&
          e.target.id !== "tools_shapelib_show"
        ) {
          var area = $("#workarea");
      ...
      shower.on("mouseup", function () {
        ...
        if (isMobile) {
          ...
          vb[1] = "165";
          if (y + height >= $(window).height() - (keyboard.height() + 90))
            area.attr("viewBox", vb.join(" "));
      ...
      function groupSymbol(
        ...
        if (sx === Infinity || sy === Infinity) scale.setScale(1, 1);
        else scale.setScale(sx, sy);
      ...
      $("#shape_buttons").mouseup(function (evt) {
        ...
        if (!btn[0].mathdata) {
          ...
          if (selectedElements.length == 0) {
            var mode = canv.getMode();
            if (mode !== mode_id) return;
            var x = (start_x = document
              .getElementById("math_cursor")
              .getAttribute("x"));
            var y = (start_y = document
              .getElementById("math_cursor")
              .getAttribute("y"));
            var cur_style = canv.getStyle();
            cur_shape = canv.addSvgElementFromJson({
            ...
            //cur_shape.setAttribute("d", current_d);
            if (cur_shape_id == "fraction" || cur_shape_id == "root")
              cur_shape.setAttribute("stroke-width", 5); //**MDP
            if (
              cur_shape_id == "sum" ||
              cur_shape_id == "lbracket" ||
              cur_shape_id == "rbracket"
            )
              cur_shape.setAttribute("fill", "black"); //**MDP
          ...
          if (
            (cur_shape_id == "root" ||
              cur_shape_id == "integral" ||
              cur_shape_id == "sum" ||
              cur_shape_id == "lbracket" ||
              cur_shape_id == "fraction") &&
            ...
            for (
              i = 0;
              i < selectedElements.length && selectedElements[i] != null;
              i++
            ) {
              ...
              if (bb_max_x < selected.x + selected.width)
                bb_max_x = selected.x + selected.width;
              if (bb_max_y < selected.y + selected.height)
                bb_max_y = selected.y + selected.height;
            }
