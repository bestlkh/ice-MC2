angular
  .module("Controllers", ["ngRoute", "ngSanitize"])
  ...
  .controller(
    "loginCtrl",
    function ($scope, $location, $rootScope, $socket, $routeParams, $window) {
      // Login Controller
      ...
      $scope.redirect = function (create) {
        ...
        if ($scope.ta || ($scope.form.username && $scope.form.roomId)) {
          if (
            $scope.ta ||
            ($scope.form.username.length <= 20 &&
              $scope.form.roomId.length <= 50)
          ) {
            $socket.emit(
              "new user",
              {
                secret: $scope.form.secret,
                username: $scope.form.username,
                userAvatar: $scope.userAvatar,
                initials: $scope.form.initials,
                roomId: $scope.form.roomId,
                isJoin: $scope.isJoin && !create,
                token: $scope.token,
              },
              function (data) {
                //console.log(data);
            ...
            // should not normally be able to reach here due to html maxlength constraint
