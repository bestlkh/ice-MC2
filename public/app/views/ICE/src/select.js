...
(function () {
  ...
  svgedit.select.Selector.prototype.resize = function () {
    ...
    if (svgedit.browser.isTouch()) {
      var gripOffset = 10;
      this.gripCoords = {
        nw: [nbax, nbay].map(Math.round).map(function (a) {
          return a + gripOffset;
        }),
        ne: [nbax + nbaw, nbay].map(Math.round).map(function (a) {
          return a + gripOffset;
        }),
        sw: [nbax, nbay + nbah].map(Math.round).map(function (a) {
          return a + gripOffset;
        }),
        se: [nbax + nbaw, nbay + nbah].map(Math.round).map(function (a) {
          return a + gripOffset;
        }),
        n: [nbax + nbaw / 2, nbay].map(Math.round).map(function (a) {
          return a + gripOffset;
        }),
        w: [nbax, nbay + nbah / 2].map(Math.round).map(function (a) {
          return a + gripOffset;
        }),
        e: [nbax + nbaw, nbay + nbah / 2].map(Math.round).map(function (a) {
          return a + gripOffset;
        }),
        s: [nbax + nbaw / 2, nbay + nbah].map(Math.round).map(function (a) {
          return a + gripOffset;
        }),
      };
    }
