...
angular
  ...
  .controller(
    ...
    function (
      ...
      var nsp = "";
      if ($location.search().nsp) nsp = "/" + $location.search().nsp;
      ...
      $scope.hambugerOpen = function () {
        if (
          !angular
            .element(document.querySelector("#chat-wrapper"))
            .hasClass("menu-open")
        ) {
          angular
            .element(document.querySelector("#chat-wrapper"))
      ...
      $socket.on("disconnect", function () {
        ...
        $scope.messages.push({
          msg: "Disconnected from chat room.",
          error: true,
          type: "system",
        });
      });
      ...
      $scope.showMessageDetails = function (message) {
        ...
        text +=
          "<br><a style='cursor:pointer;' onclick='showMessageRawNewWindow(\"" +
          btoa(
        ...
        text +=
          "<b>Text Size: </b>" + message.getText().getTextSize() + " bytes<br>";
        text +=
          "<b>Attachments Size: </b>" +
          message.getText().getAttachmentsSize() +
          " bytes";
      ...
      $scope.editSvgSource = function (source) {
        Alert.Confirm.spawn(
          "Are you sure?",
          "This will discard your current drawings.",
          function () {
            var layer = $("#editor-frame")
              .contents()
              .find("#svgcontent .active-layer");
            layer.html(source);
            if ($(window).width() < 732) {
              swapFrame();
            }
      ...
      $scope.$watch("upload.image", function () {
        ...
        ImageUtility.iOSOrientationAutoFix(
          $scope.upload.image,
          function (url) {
            let img = new Image();
            img.onload = function () {
          ...
          if ($scope.messages[i].hasFile) {
            if ($scope.messages[i].istype === "image") {
              if ($scope.messages[i].dwid === search_id) {
                $http
                  .post("/v1/getfile", $scope.messages[i])
      ...
      $scope.referenceMessage = function (msg) {
        chatInput.addBlock(
          "Reply to " + msg.getUsername() + msg.getText().getRaw(),
        );
