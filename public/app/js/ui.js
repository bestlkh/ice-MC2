...
if (
  ...
  alert(
    "Your browser is not supported by this application, please install one of the following browsers:\n" +
...
onMainLoop(function () {
  ...
  if ($("#text-message-input-area").hasClass("latex-editor-shown")) {
    if ($(window).width() > 732) {
      ...
      $.each(
        $(".message-tool-button")
          .not("#send-message-button")
          .not("#equation-editor-button")
          .not("#text-message-input-extra-symbols .latex-editor-symbol-button"),
        function (key, val) {
    ...
    if ($(window).width() > 732) {
      ...
      $.each(
        $(".message-tool-button")
          .not(".latex-editor-symbol-button")
          .not("#equation-editor-button"),
        function (key, val) {
          toolButtonTotalWidth += $(val).outerWidth();
        },
      ...
      // Include all tool buttons
      $.each(
        $(".message-tool-button").not(".latex-editor-symbol-button"),
        function (key, val) {
          toolButtonTotalWidth += $(val).outerWidth();
        },
      );
...
showMessageRawNewWindow = function (raw) {
  window
    .open()
    .document.write(
      "<pre style='word-wrap: break-word'>" + atob(raw) + "</pre>",
    );
};

function initializeChatMenu() {
  chatMenu = new UI.BubbleMenu(
    "#chat-menu",
    "#chat-menu-toggle",
