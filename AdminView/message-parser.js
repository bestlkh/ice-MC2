...
Student.prototype.toCSV = function () {
  return {
    ...
    utorid: this.utorid,
    "total messages": this.getTotalMessages(),
...
var parseStudents = function (db, sessionId, callback) {
  db.collection("chatHistory").findOne(
    { sessionId: sessionId, deleted: null },
    function (err, history) {
      ...
      session.messages.forEach(function (message) {
        ...
        if (!findOne(sess.students, { utorid: message.utorid }))
          sess.students.push(new Student(message));
