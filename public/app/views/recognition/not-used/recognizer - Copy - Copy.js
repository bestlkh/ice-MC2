...
function getExpression() {
  ...
  for (i = 0; i < eqns.length; i++) {
    if (eqns[i].textContent == " ") continue;
...
function parse(ls) {
  ...
  while (queue.length != 0) {
    while (queue.length != 0) {
      ...
      r = [
        [ls[temp1].wall[0][0], ls[temp1].wall[0][1]],
        [ls[temp1].wall[1][0], ls[temp1].wall[1][1]],
      ];
      temp2 = hor(ls, temp1);
      while (temp2 != -1) {
        ...
        ls[temp2].wall = [
          [oldWall[0][0], oldWall[0][1]],
          [oldWall[1][0], oldWall[1][1]],
        ];
        symbol = {
          symbol: ls[temp2],
      ...
      while (stack.length != 0) {
        ...
        var right =
          si == parent.children.length - 1
            ? r[1][0]
            : parent.children[si + 1].symbol.minX;
...
function start(ls, r) {
  ...
  while (i < n && limitIndex == -1) {
    if (!ls[i].marked && ls[i].type === "LIMIT" && isInRegion(r, ls[i]))
      limitIndex = i;
    else i += 1;
  }
  ...
  while (index < ls.length) {
    if (isInRegion(ls[s].wall, ls[index]) && !ls[index].marked) {
      if (ls[s].maxX <= ls[index].minX) {
        if (
          ls[s].minY + (1 / 5) * ls[s].height <= ls[index].y &&
          ls[index].y < ls[s].maxY - (ls[s].height * 2) / 5
        )
