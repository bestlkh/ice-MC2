        ...
        wolfram.query(match[1].replace(/\$+/g, "")).then(function (result) {
          ...
          streamToBuffer(renderStream, function (err, buffer) {
            ...
            wolfbot.emit(
              "send-message",
              {
                msg: imgTemplate.replace("{url}", datauri.content),
                hasMsg: true,
              },
              function (result) {},
            );
...
var parseResult = function (result) {
  ...
  result.forEach(function (pod) {
    html +=
      "<p style='color: #909090; font-family: Helvetica Neue, Helvetica, Arial, sans-serif;'>" +
      pod.title +
      "</p>";
    pod.subpods.forEach(function (sub) {
      if (sub.image)
        html +=
          "<img style='max-width: 290px;' src='" + sub.image + "'/><p></p>";
    });
  });

  html =
    "<div style='background-color: #fff; padding: 5px; margin: auto;'>" +
    html +
    "</div>";
  return html;
};
