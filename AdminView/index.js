...
AdminView.prototype.connectToDb = function (callback) {
  try {
    sequelize.authenticate();
    console.log(
      "[AdminView ConnectToDb] Connection has been established successfully to the database.",
    );
    sequelize.sync({
      alter: {
        drop: false,
...
AdminView.prototype.setupApi = function () {
      ...
      this.db.collection("users").findOne(
        ...
        function (err, user) {
          if (err)
            return res
              .status(500)
              .end("Server error, could not resolve request");
  ...
  this.app.get(
    "/v1/api/classrooms/:name/students/tokens.csv",
    checkAuth,
    function (req, res) {
        ...
        function (err, result) {
          ...
          if (result && result.students) {
            this.db
              ...
              .findOne(
                { owner: req.session.user.username, name: req.params.name },
                function (err, classroom) {
                  ...
                  return csv
                    .stringify(result.students, { header: true })
                    .pipe(res);
                },
              );
  ...
  this.app.get(
    ...
    function (req, res) {
      this.db
        .collection("classrooms")
        .find({ owner: req.session.user.username })
        .toArray(function (err, crs) {
          if (err)
            return res.status(500).json({
              status: 500,
              message: "Server error, could not resolve request",
  ...
  this.app.patch(
    ...
    function (req, res) {
      ...
      this.db.collection("classrooms").findOne(
        { roomName: classroom.roomName, owner: req.session.user.username },
        function (err, cr) {
          if (err)
            return res.status(500).json({
              status: 500,
          ...
          if (cr && cr.name !== classroom.name)
            return res.status(400).json({
              status: 400,
              message: "Classroom with room name already exist",
            });
  ...
  this.app.post(
    ...
    function (req, res) {
      ...
      this.db.collection("classrooms").findOne(
        {
          owner: req.session.user.username,
          $or: [{ name: classroom.name }, { roomName: classroom.roomName }],
        },
        function (err, cr) {
          if (err)
            return res.status(500).json({
            ...
            return res.status(400).json({
              status: 400,
              message: "Classroom with that name/room name already exists",
            });
          this.db
            .collection("classrooms")
            .insertOne(classroom, function (err, result) {
              if (err)
                return res.status(500).json({
                  status: 500,
                  message: "Server error, could not resolve request",
        ...
        function (err, result) {
          ...
          this.db.collection("students").removeOne(
            ...
            function (err, result) {
              ...
              this.db.collection("chatHistory").update(
                {
                  className: req.params.name,
                  owner: req.session.user.username,
                },
  ...
  this.app.get(
    "/v1/api/classrooms/:name/students",
    checkAuth,
    function (req, res) {
      this.db.collection("students").findOne(
        {
  ...
  this.app.post(
    ...
    function (req, res) {
      csv.parse(
        Buffer.from(req.body.csv, "base64"),
        { columns: true },
        function (err, data) {
          if (err)
            return res
              .status(400)
              .json({ status: 400, message: "Invalid csv format" });

          let operation =
            req.body.mode == 0
              ? { $set: { students: data } }
              : { $push: { students: { $each: data } } };
          this.db
            .collection("students")
            .updateOne(
              { owner: req.session.user.username, className: req.params.name },
              operation,
              { upsert: true },
              function (err, result) {
                if (err)
      ...
      function (err, classroom) {
        ...
        this.db.collection("students").findOne(
          ...
          function (err, students) {
            ...
            let student = findOne(students.students, {
  ...
  this.app.get(
    "/v1/api/namespace/:nsp/room/:roomName/track/:code",
    getUserTracking,
    function (req, res) {
  ...
  this.app.patch(
    "/v1/api/classrooms/:name/students/generate",
    checkAuth,
    function (req, res) {
      this.db.collection("students").findOne(
        {
        ...
        function (err, students) {
          ...
          if (!students)
            return res
              .status(400)
              .json({ status: 400, message: "Invalid student list" });
          let newStudents = [];
          let urls = {};
          students.students.forEach(function (student) {
  ...
  this.app.post(
    ...
    function (req, res) {
      ...
      if (!ta.name || ta.name === "")
        return res
          .status(400)
          .json({ status: 400, message: "Invalid TA name" });

      this.db.collection("ta").findOne(
        { owner: ta.owner, name: ta.name },
        function (err, result) {
          if (err)
            return res.status(500).json({
              status: 500,
          ...
          if (result)
            return res
              .status(400)
              .json({ status: 400, message: "Name already exists" });
  ...
  this.app.get(
    ...
    function (req, res) {
      this.db
        .collection("ta")
        .find({ owner: req.session.user.username }, { avatar: 0, owner: 0 })
  ...
  this.app.post(
    ...
    function (req, res) {
      if (!req.file)
        return res.status(400).json({ status: 400, message: "Invalid image" });
  ...
  this.app.get(
    "/v1/api/classrooms/:name/sessions",
    checkAuth,
    function (req, res) {
      this.db
  ...
  this.app.get(
    "/v1/api/classrooms/:name/sessions/:id/messages",
    checkAuth,
    function (req, res) {
      if (isNaN(req.params.id))
        return res.status(400).json({ status: 400, message: "Invalid id" });
      this.db.collection("chatHistory").findOne(
        {
          owner: req.session.user.username,
          className: req.params.name,
  ...
  this.app.get(
    "/v1/api/classrooms/:name/sessions/:id/students",
    checkAuth,
    function (req, res) {
      if (isNaN(req.params.id))
        return res.status(400).json({ status: 400, message: "Invalid id" });
  ...
  this.app.get(
    "/v1/api/classrooms/:name/sessions/messages.csv",
    checkAuth,
    function (req, res) {
      this.db
        .collection("chatHistory")
        ...
        .toArray(function (err, sessions) {
          ...
          res.writeHead(200, {
            ...
            "Access-Control-Allow-Origin": "*",
            "Content-Disposition":
              "attachment; filename=messages-" + req.params.name + ".csv",
          });
          replaceImages(messages);
          replaceTimestamp(messages);
          return csv
            .stringify(messages, { columns: messageColumns, header: true })
            .pipe(res);
        });
    }.bind(this),
  );
  ...
  this.app.get(
    "/v1/api/classrooms/:name/sessions/:id/messages.csv",
    checkAuth,
    function (req, res) {
      if (isNaN(req.params.id))
        return res.status(400).json({ status: 400, message: "Invalid id" });
  ...
  var replaceImages = function (messages) {
    messages.forEach(function (message) {
      if (message.msg.startsWith("[mc2-image]")) message.msg = "[mc2-image]";
    });
  };

  var replaceTimestamp = function (messages) {
    messages.forEach(function (message) {
      message.timestamp = moment(message.timestamp).format(
        "MM/DD/YYYY hh:mm:ss",
      );
    });
  };
  ...
  this.app.get(
    "/v1/api/classrooms/:name/sessions/students.csv",
    checkAuth,
    function (req, res) {
      this.db
        .collection("chatHistory")
            ...
            Promise.all(
              ...
              .then(function () {
                res.writeHead(200, {
                  ...
                  "Access-Control-Allow-Origin": "*",
                  "Content-Disposition":
                    "attachment; filename=students-" + req.params.name + ".csv",
                });
  ...
  this.app.get(
    "/v1/api/classrooms/:name/sessions/:id/students.csv",
    checkAuth,
    function (req, res) {
      if (isNaN(req.params.id))
        return res.status(400).json({ status: 400, message: "Invalid id" });
      parseStudent(this.db, parseInt(req.params.id), function (err, result) {
        ...
        res.writeHead(200, {
          ...
          "Access-Control-Allow-Origin": "*",
          "Content-Disposition":
            "attachment; filename=students-" + req.params.id + ".csv",
        });
          ...
          socket.handshake.session.reload(
            function (err) {
              ...
              if (socket.handshake.session.isAdmin) {
                ...
                if (
                  !this.ios.sockets.adapter.rooms[
                    session.settings.chat.roomName
                  ]
                )
                  return callback({
                    status: "Offline",
