...
ChatNsp.prototype.listen = function () {
  if (!(this.onLogin && this.onJoin))
    return console.log(
      "Failed to start listening: need onLogin and onJoin functions set",
    );
  this.nsp.on(
    "connection",
    function (socket) {
...
LectureNsp.prototype.findRoom = function (roomName, callback) {
  const room = Rooms.findOne({ where: { name: roomName } });
  if (!room) {
...
LectureNsp.prototype.findStudent = function (data, callback) {
  const student = Student.findOne({
    where: { name: this.owner, RoomId: data.roomId },
  });
  if (!student) {
...
LectureNsp.prototype.findTa = function (data, callback) {
  const ta = Ta.findOne({ where: { name: this.owner, RoomId: data.roomId } });
  if (!ta) {
      ...
      this.nsp.on(
        ...
        function (socket) {
          ...
          function setSessionVar(variable, value) {
            ...
            if (socket.handshake.session.chatVars)
              socket.handshake.session.chatVars.push(variable);
          ...
          socket.on(
            ...
            function (data, callback) {
              var history = this.findRoomAdapter(
                socket.connectedRoom,
              ).messageHistory;
              var index = history.findIndex(function (item, i) {
                return (
                  item.msgTime === data.msgTime &&
                  ...
                  if (this.findClient(data.roomId, data.username)) {
                    ...
                    var login = function (data) {
                      if (data.userAvatar && isNaN(data.userAvatar))
                        return callback({
                          success: false,
                          message: "Invalid user avatar",
                        });
                      else
                        data.userAvatar = "Avatar" + data.userAvatar + ".jpg";
                      if (
                        socket.handshake.session.userAvatar &&
                        !data.userAvatar
                      )
            ...
            function (data, callback) {
              ...
              this.findRoom(
                ...
                function (err, classroom) {
                  ...
                  if (
                    nameExists &&
                    !socket.handshake.session.isInstructor &&
                    !socket.handshake.session.isAdmin
                  ) {
                    destroySession();
                    return callback({
                      success: false,
                  ...
                  if (socket.handshake.session.username) {
                    if (
                      classroom &&
                      classroom.invite &&
                      !socket.handshake.session.ta &&
                      !socket.handshake.session.isInstructor &&
                      !socket.handshake.session.utorid &&
                      !socket.handshake.session.isAdmin
                    ) {
                      destroySession();
                      return callback({
                    ...
                    if (
                      socket.handshake.session.utorid &&
                      socket.handshake.session.connectedClass
                    ) {
                      if (
                        data.roomId !== socket.handshake.session.connectedClass
                      ) {
                        return callback({
                          success: false,
                          message: "Log in again to access room",
                        });
                      }
                    }
                    ...
                    socket.join(
                      ...
                      function () {
                        ...
                        if (!room.sessionId)
                          this.setRoomAdapterVars(data.roomId, {
                            sessionId: classroom.sessionId,
                            className: classroom.name,
                          });

                        if (!this.rooms[data.roomId])
                          this.rooms[data.roomId] = Object.assign({}, room);
          ...
          socket.on(
            ...
            function (data) {
              ...
              for (var clientId in i.sockets) {
                temp1 = {
                  username:
                    this.nsp.connected[clientId].handshake.session.username,
                  userAvatar:
                    this.nsp.connected[clientId].handshake.session.userAvatar,
                };

                if (socket.handshake.session.isAdmin) {
                  temp1.utorid =
                    this.nsp.connected[clientId].handshake.session.utorid;
                }
                if (
                  this.nsp.connected[clientId].handshake.session.isAdmin ||
                  this.nsp.connected[clientId].handshake.session.isInstructor
                ) {
          ...
          socket.on(
            ...
            function (data, callback) {
              ...
              if (socket.handshake.session.username) {
                ...
                this.findRoomAdapter(socket.connectedRoom).messageHistory.push(
                  message,
                );
          ...
          socket.on(
            ...
            function (data, callback) {
              ...
              if (socket.handshake.session.username) {
                ...
                var message = new Message.ImageMessage(
                  data,
                  socket.handshake.session,
                );
          ...
          socket.on(
            ...
            function (data, callback) {
              ...
              if (socket.handshake.session.settings)
                isInstructor =
                  socket.handshake.session.settings.chat.roomName.toLowerCase() !==
                  data.roomId;
              if (session.user) {
                setSessionVars({
                ...
                setSessionVar(
                  "redirectTo",
                  "/#/v1/" + data.roomName + "?nsp=" + this.owner,
                );
                callback({ success: true });
              }
            }.bind(this),
            ...
            function (callback) {
              this.sendMessage(
                socket.connectedRoom,
                {
                  username: "[System]",
                  msg:
                    socket.handshake.session.username + " has left the room.",
                  timestamp: moment().valueOf(),
                  type: "system",
                  hidden: true,
                },
                function (err, result) {
                  if (!this.findRoomAdapter(socket.connectedRoom))
                    delete this.rooms[socket.connectedRoom];
                }.bind(this),
              );
