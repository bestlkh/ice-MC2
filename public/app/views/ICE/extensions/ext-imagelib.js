...
methodDraw.addExtension("imagelib", function () {
  ...
  function reloadAccreditedLibraries() {
    return new Promise(function (resolve, reject) {
      axios
        .get(ACCREDITED_LIB_ENDPOINT)
        .then(function (data) {
          accreditedLibs = data.data;
          resolve(data.data);
        })
        .catch(function (e) {
          PARENT_WINDOW.Alert.Notification.spawn(
            "Failed to load accredited image libraries...",
            "error",
          );
          reject(e);
        });
  ...
  function promptExternalImport(e) {
    var url = prompt(
      "Please enter your external library URL\nex. https://example.com/lib/my-img-lib.json",
    );
    if (url) {
      importExternalLibrary(url)
        .then(function (info) {
          alert(
            "Successfully imported external library: " +
              info.name +
              " by " +
              info.author +
              ".",
  ...
  function makeImageLibraryButton(lib, image) {
    let button = $(
      "<div class='imglib-browser-button ignore-touch-conversion'>",
    );
    ...
    button.append(
      "<div class='imglib-button-overlay ignore-touch-conversion'>" +
        image.title +
        "</div>",
    );
  ...
  function makeImageLibraryButtons() {
    ...
    for (var i = 0; i < loadedLibs.length; i++) {
      ...
      if (
        currentSelectedLibrary !== "All" &&
        currentSelectedLibrary !== loadedLibs[i].name
      ) {
        continue;
      }
      for (var k = 0; k < loadedLibs[i].images.length; k++) {
        buttons.push(
          makeImageLibraryButton(loadedLibs[i], loadedLibs[i].images[k]),
        );
      }
    }
    return buttons;
  ...
  function makeImageLibraryPanel() {
    let panel = $(
      "<div id='imglib-panel' class='ignore-touch-conversion'></div>",
    );
    ...
    let panelBrowser = $(
      "<div id='imglib-browser' class='ignore-touch-conversion'>",
    );
