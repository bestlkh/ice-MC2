...
$.SvgCanvas = function (container, config) {
  ...
  var clearSvgContentElement = (canvas.clearSvgContentElement = function () {
    ...
    var comment = svgdoc.createComment(
      " iCE: interactive Collaborative Editor ",
    );
  ...
  var addToSelection = (this.addToSelection = function (elemsToAdd, showGrips) {
    ...
    if (
      showGrips ||
      (selectedElements.length == 1 && selectedElements[0].nodeName != "text")
    )
      selectorManager.requestSelector(selectedElements[0]).showGrips(true);
    //**MDP to prevent text from resizing
  ...
  (function () {
    ...
    var mouseMove = function (evt) {
      ...
      if (selectedElements.length > 0) $(".tools_flyout").hide();
      /*
  ...
  */
          ...
          if (selectedElements[0] != null) {
            ...
            if (selectedElements.length == 1) {
              ...
              switch (selected.tagName) {
                ...
                default:
                  cur_properties.fill =
                    selected.getAttribute("fill") == "red"
                      ? selected.getAttribute("backup-fill")
                      : selected.getAttribute("fill");
                  cur_properties.fill_opacity =
                    selected.getAttribute("fill-opacity");
                  cur_properties.stroke =
                    selected.getAttribute("stroke") == "red"
                      ? selected.getAttribute("backup-stroke")
              ...
              if (selected.tagName == "text") {
                ...
                if (selected.textContent == "\u222b")
                  selectorManager.requestSelector(selected).showGrips(true); //**MDP RESIZEABLE
                if (selected.textContent == "\u2211")
                  selectorManager.requestSelector(selected).showGrips(true); //**MDP
              } else {
                //**MDP
            ...
            //MDP -- Math Cursor Mode on click and swipe to move cursor
            if (
              Math.abs(real_x - r_start_x) < 5 &&
              Math.abs(real_y - r_start_y) < 5
            ) {
              placeSnapPoints();
              ...
              for (var i = 0; i < snaps.length; i++) {
                ...
                if (
                  0 <= diffX &&
                  diffX < widthThreshold &&
                  0 <= diffY &&
                  diffY < heightThreshold
                ) {
                  var x = snapX + 1,
                    y = snapY;
                  if (type == "contains") {
      ...
      if (selected != null) {
        if (elementIn) {
          if (direction > 0) {
            ...
            if (
              getBBox(selected).x + dx - (2 * getBBox(selected).width) / 3 >
              curX
            ) {
              break;
            } else {
        ...
        if (!elementIn) selectorManager.requestSelector(selected).resize();
      }
    }
    if (!batchCmd.isEmpty()) {
  ...
  this.moveCursor = function (dx, dy) {
    ...
    if (w != null) {
      ...
      if (dx != 0) {
        ...
        var func = function (symbol) {
          symbol = getBBox(document.getElementById(symbol.id));
          if (Math.abs(y - symbol.y) > 18) return;
      ...
      if (dy != 0) {
        ...
        var func = function (symbol) {
          symbol = getBBox(document.getElementById(symbol.id));
          if (Math.abs(x - symbol.x) > 25) return;
  ...
  this.pullAllAtCursor = function (width, excl) {
    ...
    var regionCondFunc = function (region) {
      var res =
        region.wall.left <= cursor_x &&
        cursor_x < region.wall.right &&
        region.wall.top <= cursor_y + 10 &&
        cursor_y + 10 < region.wall.bottom;
  ...
  this.removeNearestToCursor = function () {
    ...
    for (var i = 0; i < eqns.length; i++) {
      ...
      if (eqn.x + eqn.width > math_cursorB.x + math_cursorB.width) {
        ...
        eqn.y + eqn.height <
        math_cursorB.y + math_cursorB.height / 2
      ) {
        continue;
      } else {
        seqns.push(eqns[i]);
    ...
    if (seqns.length == 0) {
      if (this.autoSpacing) pullAllAtCursor();
      moveCursor(-1, 0);
      return;
    }
    ...
    if (math_cursor.getAttribute("x") - (getBBox(t).x + getBBox(t).width) > 5) {
      ...
      if (this.autoSpacing) pullAllAtCursor(-1 * getBBox(t).width);
  ...
  this.addToSVG = function (selectedSymbol) {
    ...
    var y =
      Number(math_cursor.getAttribute("y")) +
      Number(math_cursor.getAttribute("height"));
  ...
  this.keyPressed = function (key) {
    ...
    if (key == " ") {
      if (this.autoSpacing) pushAllAtCursor();
      moveCursorAbs(10, 0);
      return;
    }
    ...
    var newChar = false;
    var shortcutsVisible =
      document.getElementById("floatingContent").style.display;
      ...
      shortcuts.forEach(function (shortcut, i) {
        if (i == shortcutIndex) {
          if (shortcuts[i].length == 1) {
            shortcutText +=
              '<div class="suggest" onmouseup="svgCanvas.addToSVG(' +
              "'" +
              shortcuts[i] +
              "'" +
              ');"> ' +
    ...
    if (newChar || !shortcuts) {
      ...
      if (shortcuts) {
        if (shortcuts[shortcutIndex].length == 1) {
          ...
          newText.textContent = String.fromCharCode(
            "0x" + shortcuts[shortcutIndex],
          );
        }
      }
    }
    ...
    var diffWidth =
      Number(bbox.x) +
      Number(bbox.width) -
      Number(math_cursor.getAttribute("x")) +
      1;
    var cloneNewText = newText.cloneNode(true);
    parentNewText.removeChild(newText);
    if (this.autoSpacing) {
      if (diffWidth > 0) pushAllAtCursor(diffWidth);
      else pullAllAtCursor(diffWidth);
    }
